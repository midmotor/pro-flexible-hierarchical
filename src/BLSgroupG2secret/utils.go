...
func (p Polynomial) Commit() []groupP {
	...
	for i := 0; i < int(p.degree+1); i++ {
		...
		polycommit[i].ScalarMultiplication(&gen2a, p.Coefficients[i].BigInt(&big.Int{}))
...
func PointinterpoleT(rfrom []bls.G1Affine, thresholdIDlist []fr.Element, t int, value fr.Element) (*bls.G1Affine, error) {
	...
	out := new(bls.G1Affine).ScalarMultiplication(&rfrom[0], lagranges[0].BigInt(&big.Int{}))

	//////////
	for i := 1; i < int(len(rfrom)); i++ {
		out.Add(out, new(bls.G1Affine).ScalarMultiplication(&rfrom[i], lagranges[i].BigInt(&big.Int{})))
...
func printEq(a bls.G2Affine, b bls.G2Affine) {

	fmt.Printf("%t", a.Equal(&b))
}
