...
func BenchmarkTC(b *testing.B) {
	...
	for _, tc := range testCases {
		...
		b.Run(tc.name+"-G1-Keygen", func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		b.Run(tc.name+"-G1-Update", func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				...
				for i := 0; i < threshold; i++ {
					...
					testPoint[i].ScalarMultiplication(&gen1a, newUpdateSecret[i].BigInt(&big.Int{}))
		...
		b.Run(tc.name+"-G1-Disenrollment", func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				zeroPolys := make([]*Polynomial, tc.n-1)
				// create polynomials with constant 0
				...
				for i := 0; i < threshold; i++ {
					testPoint[i].ScalarMultiplication(&gen1a, newUpdateSecret[i].BigInt(&big.Int{}))
					//testPoint[i] = suite.G1().Point().Mul(newUpdateSecret[i], nil)
				}
		...
		b.Run(tc.name+"-G1-Enrollment", func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				...
				coms := make([][]bls.G1Affine, threshold)
				for i := range exponents {
					coms[i] = make([]bls.G1Affine, threshold)
				...
				for k := 0; k < threshold; k++ {
					...
					for i := 0; i < threshold; i++ {
						...
						for j := 1; j < threshold; j++ {
							...
							temp.Add(&temp, new(bls.G1Affine).ScalarMultiplication(&coms[i][j], exponents[k][j].BigInt(&big.Int{})))

						}
						new(bls.G1Affine).ScalarMultiplication(&gen1a, newCollects[k][i].BigInt(&big.Int{})).Equal(&temp)
					}
				}
		...
		b.Run(tc.name+"-G1-ThChange", func(b *testing.B) {
		...
		/*
			...
			var sigmas1 = make([]bls.G2Jac, threshold)
			b.Run(tc.name+"-G1-Boldy1-Sign", func(b *testing.B) {

				sigmasAff := make([]bls.G2Affine, threshold)
				b.ResetTimer()
			...
			b.Run(tc.name+"-G1-Boldy1-Verify", func(b *testing.B) {
		...
		*/
		...
		var sigmas2 = make([]bls.G2Jac, threshold)
		var proofs = make([]Pf, threshold)
		roMsgAf, _ := bls.HashToG2([]byte(msg), []byte("DST"))
		var roMsg = *new(bls.G2Jac).FromAffine(&roMsgAf)

		var sigma = new(bls.G2Affine)
		b.Run(tc.name+"-G1-Boldy2-PSignGen", func(b *testing.B) {
			sigmasAff := make([]bls.G2Affine, threshold)
			...
			for i := 0; i < b.N; i++ {
				for i := 0; i < len(tList); i++ {
					...
					proofs[i] = cpProve(*new(bls.G1Jac).FromAffine(&keygenR3s[i].Public), roMsg, sigmas2[i], secrets[i])
		...
		b.Run(tc.name+"-G1-Boldy2-PSignAgg and Sign Agg", func(b *testing.B) {

			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				for i := 0; i < len(tList); i++ {
					PVerify([]byte(msg), sigmas2[i], keygenR3s[i].Public)
					cpVerify(*new(bls.G1Jac).FromAffine(&keygenR3s[i].Public), roMsg, sigmas2[i], proofs[i])
				}
			}

		})

		b.Run(tc.name+"-G1-Boldy2-Verify", func(b *testing.B) {

			b.ResetTimer()
			PVerify([]byte(msg), *new(bls.G2Jac).FromAffine(sigma), masterPub)
