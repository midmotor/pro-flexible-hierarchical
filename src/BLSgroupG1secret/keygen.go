...
func CreateSch(ID fr.Element, value groupP, secret fr.Element) *schnorr {
	...
	randomexponent := new(groupP).ScalarMultiplication(&gen1a, k.BigInt(&big.Int{}))
...
func (s *schnorr) Verify(fromID fr.Element) error {
	...
	// proof.G + c.(s.G) =? k.G
	lhs1 := new(groupP).ScalarMultiplication(&gen1a, s.proof.BigInt(&big.Int{}))
...
func CreateKeyGenR3(secret fr.Element) *KeygenR3 {
	...
	return &KeygenR3{*new(groupP).ScalarMultiplication(&gen1a, secret.BigInt(&big.Int{}))}
...
func Sscheck(ownerID fr.Element, sskey fr.Element, com []groupP, t int) error {
	...
	lhs := new(groupP).ScalarMultiplication(&gen1a, sskey.BigInt(&big.Int{}))
